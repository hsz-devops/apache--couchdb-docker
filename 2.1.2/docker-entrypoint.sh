...
if [ "$1" = '/opt/couchdb/bin/couchdb' ]; then
	...
	chown -R couchdb:couchdb /opt/couchdb       || true

	chmod -R 0770 /opt/couchdb/data             || true

	chmod 664 /opt/couchdb/etc/*.ini
	chmod 664 /opt/couchdb/etc/default.d/*.ini
	# # find /opt/couchdb/etc -name \*.ini -exec chmod -f 664 {} \;
	# find /opt/couchdb/etc -name \*.ini -exec chmod -f 664 {} || true \;
	...
	# make sure that CouchDB will write custom settings in this file
	touch /opt/couchdb/etc/local.d/zzzz-writeable-configs.ini
	chown -f couchdb:couchdb /opt/couchdb/etc/local.d/zzzz-writeable-configs.ini || true
